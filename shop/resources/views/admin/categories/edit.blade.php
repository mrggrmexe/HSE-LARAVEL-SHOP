...
@section('content')
    <div class="mx-auto max-w-3xl rounded-xl bg-white p-6 shadow">
        <h1 class="text-3xl font-bold">Редактирование категории</h1>
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="mt-8 space-y-5">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2">
            </div>

            <div>
                <label for="slug" class="mb-2 block text-sm font-medium">Slug</label>
                <input type="text" id="slug" name="slug" value="{{ old('slug', $category->slug) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2">
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" class="w-full rounded-lg border border-gray-300 px-3 py-2">{{ old('description', $category->description) }}</textarea>
            </div>
